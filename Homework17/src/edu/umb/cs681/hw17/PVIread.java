...
public class PVIread {
    ...
    public static void PVIaverage() {
        ...
        float[] PVI_mean = MA_counties_PVI_data.stream().parallel().map((List<String> county) -> new String[] {county.get(0), county.get(5), county.get(6), county.get(9)}).reduce(
                new float[5],
                (result, pvi) -> {
    ...
    public static void PVIstd() {
        ...
        float PVI_mean = MA_counties_PVI_data.stream().parallel().map((List<String> county) -> county.get(0)).reduce(
                new float[2],
                (result, pvi) -> {
        ...
        float PVI_variance = MA_counties_PVI_data.stream().parallel().map((List<String> county) -> county.get(0)).reduce(
                new float[2],
                (result, pvi) -> {
